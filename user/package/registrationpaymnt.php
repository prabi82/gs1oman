...
if(isset($_REQUEST['_token']) && !empty($_REQUEST['_token'])) {
	...
	$offline_payment = $orderData['offline_payment'];
	
	if($offline_payment=='1'){
		$encoded='offline'.$_token;
		$uid = $orderData['id'];
		$nextYear =  date('Y');
	   $addTwoYears =  date('Y', strtotime('+1 year'));
	   $renewalDate=$addTwoYears.'-01-01';
	   $expiryDateDb= $nextYear.'-12-31';
		$sql2 =mysqli_query($conn,"update `order_tbl` set status='1',payment_data='".$encoded."',expiry_date= '".$expiryDateDb."' where id ='".$uid."'");
		//echo "<script>window.location='thanks.php';</script>";
		echo "<script>window.location='https://gs1oman.com/thanks.php';</script>";
	...
	$RuRL = 'https://gs1oman.com/thanks.php';
	//$CuRL = 'https://gs1oman.com/user/package/cancel.php';
	$CuRL = 'https://gs1oman.com/regpaymentcancel.php';
	...
	echo '<script>window.location="registrationpaymnt.php?page=Pack";</script>';
