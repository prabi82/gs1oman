...
<?php 
	...
	if(isset($_POST['submit'])){
		...
		try {
			...
			$required_fields = ['name', 'name_ar', 'pobox', 'zipcode', 'address', 'address_ar', 'country', 'city', 'mobile_number', 'phone_number', 'cr_number', 'cr_legal_type', 'user_email', 'business_type_product_category'];
			...
			$business_type_product_category = mysqli_real_escape_string($conn, isset($_POST['business_type_product_category']) ? $_POST['business_type_product_category'] : '');
			error_log("Business Type Product Category: " . $business_type_product_category);
			...
			// Get Riyada certificate data
			$riyada_certificate = mysqli_real_escape_string($conn, isset($_POST['riyada_certificate']) ? $_POST['riyada_certificate'] : NULL);
			$exp_date = mysqli_real_escape_string($conn, isset($_POST['exp_date']) ? $_POST['exp_date'] : NULL);

			// Handle Riyada documents
			$riyada_doc = '';
			if(isset($_FILES['documents_req']) && $_FILES['documents_req']['error'] == 0) {
				$riyada_doc = $upload_path . time() . '_' . $_FILES['documents_req']['name'];
				move_uploaded_file($_FILES['documents_req']['tmp_name'], $riyada_doc);
			}
			...
			$sql = "INSERT INTO `company_tbl` (
				...
				riyada_certificate, exp_date, documents_req,
				status, record_date
			) VALUES (
				?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '0', CURDATE()
			)";
			...
			mysqli_stmt_bind_param($stmt, "sssssssssssssssssssssssssss", 
				...
				$mobile_number, $phone_number, $fax_number, $website_address, $cr_number, 
				$cr_legal_type, $cr_registration_date, $cr_expiry_date, $user_email, 
				$product_id, $business_type_product_category, $number_of_employee, $healthcare_status,
				$cr_doc, $chamber_doc, $other_doc, $riyada_certificate, $exp_date, $riyada_doc
			);
			...
			<form name="listForm" action="" method="post" enctype="multipart/form-data" id="regform">
				...
				<div class="row">
					<div class="col-md-6">
						...
						<select name="business_type_product_category" class="form-control validate" required>
