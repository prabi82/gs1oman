...
function generateRandomVATNumber() {
    // Generate a valid VAT number format for Oman (15 characters)
...
function generateRandomTaxNumber() {
    // Generate a valid Tax Registration Number (8 digits)
    ...
    test('Complete registration form submission', async () => {
        try {
            ...
            const formIsValid = await page.evaluate(() => {
                ...
                const select = form.querySelector('select[name="business_type_product_category"]');
                console.log('Business type value:', select ? select.value : 'not found');
                return form.checkValidity() && (!select || select.value);
            });
            ...
            try {
                ...
                if (!formIsValid) {
                    throw new Error('Form validation failed - required fields are empty');
                }

                // Set up response handler
                const responsePromise = page.waitForResponse(
                    response => response.url().includes('index.php') && 
                ...
                await page.evaluate(() => {
                    ...
                    // Ensure business type is included
                    const businessType = document.querySelector('select[name="business_type_product_category"]').value;
                    if (businessType) {
                        formData.set('business_type_product_category', businessType);
                    }
                ...
                if (!responseData.success) {
                    throw new Error(`Form submission failed: ${responseData.message}`);
                }
