...
if(isset($_POST['submit']))
{
    ...
    // Properly handle Riyada certificate and expiry date
    $riyada_certificate = !empty($_POST['riyada_certificate']) ? trim($_POST['riyada_certificate']) : NULL;
    $exp_date = !empty($_POST['exp_date']) ? trim($_POST['exp_date']) : NULL;
    
    error_log("Processed Riyada Values:");
    error_log("Certificate: " . ($riyada_certificate ? $riyada_certificate : 'NULL'));
    error_log("Expiry Date: " . ($exp_date ? $exp_date : 'NULL'));
...
if($status==0){
  $email_temp.='<span style="color:#ff9900">Once your application is approved, you will get  an activation email with the login details.</span>'; 
}
elseif($status==1){
...
$query1=mysqli_query($conn,$sql1)or die(mysqli_error($conn));
	...
	if (!empty($doc_name1)) {
		$relative_path = 'images/Upload/' . $doc_name1;
		$absolute_path = $_SERVER['DOCUMENT_ROOT'] . '/' . $relative_path;
		if (move_uploaded_file($doc_tmp_name1, $absolute_path)) {
		// File uploaded successfully, update the database with the relative path
		$sql1 = "UPDATE `company_tbl` SET `upload_document1`='$relative_path' WHERE id='$view_id'";
		$query1 = mysqli_query($conn, $sql1) or die(mysqli_error($conn));
...
$query2=mysqli_query($conn,$sql2)or die(mysqli_error($conn)); 
	...
	if (!empty($doc_name2)) {
		$relative_path2 = 'images/Upload/' . $doc_name2;
		$absolute_path2 = $_SERVER['DOCUMENT_ROOT'] . '/' . $relative_path2;

		if (move_uploaded_file($doc_tmp_name2, $absolute_path2)) {
			// File uploaded successfully, update the database with the relative path
			$sql1 = "UPDATE `company_tbl` SET `upload_document2`='$relative_path2' WHERE id='$view_id'";
			$query1 = mysqli_query($conn, $sql1) or die(mysqli_error($conn));
			//echo 'Records updated successfully.';
		} else {
			// Something went wrong with the file upload
...
$query3=mysqli_query($conn,$sql3)or die(mysqli_error($conn)); 
	...
	if (!empty($doc_name3)) {
		$relative_path3 = 'images/Upload/' . $doc_name3;
		$absolute_path3 = $_SERVER['DOCUMENT_ROOT'] . '/' . $relative_path3;

		if (move_uploaded_file($doc_tmp_name3, $absolute_path3)) {
			// File uploaded successfully, update the database with the relative path
			$sql1 = "UPDATE `company_tbl` SET `upload_document3`='$relative_path3' WHERE id='$view_id'";
	...
	//upload documents_req
	if(!empty($doc_name4)){
		
		$relative_path4 = 'images/Upload/' . $doc_name4;
		$absolute_path4 = $_SERVER['DOCUMENT_ROOT'] . '/' . $relative_path4;
		
		

		// Check if the file has been uploaded successfully
		if (move_uploaded_file($doc_tmp_name4, $absolute_path4)) {
			// File uploaded successfully, update the database with the relative path
			 $sql1 = "UPDATE `company_tbl` SET `documents_req`='$relative_path4' WHERE id='$view_id'";
...
// Add debug logging before SQL update
error_log("DEBUG - Before SQL Update:");
error_log("Riyada Certificate: " . var_export($riyada_certificate, true));
error_log("Expiry Date: " . var_export($exp_date, true));
error_log("View ID: " . var_export($view_id, true));
...
`riyada_certificate`=" . ($riyada_certificate ? "'$riyada_certificate'" : "NULL") . ",
`exp_date`=" . ($exp_date ? "'$exp_date'" : "NULL") . ",
`status`='$status' 
WHERE id='$view_id'";

error_log("DEBUG - SQL Query: " . $sql4);

$query4=mysqli_query($conn,$sql4)or die(mysqli_error($conn));
error_log("DEBUG - SQL Error (if any): " . mysqli_error($conn));
...
$cumpany_sql = mysqli_query($conn, "SELECT 
    *,
    CASE 
        WHEN riyada_certificate IS NULL OR riyada_certificate = '' THEN ''
        WHEN LOWER(TRIM(riyada_certificate)) = 'yes' THEN 'Yes'
        WHEN LOWER(TRIM(riyada_certificate)) = 'no' THEN 'No'
        ELSE TRIM(riyada_certificate)
    END as riyada_status,
    DATE_FORMAT(COALESCE(exp_date, CURRENT_DATE), '%Y-%m-%d') as exp_date_formatted
    FROM company_tbl WHERE id='".$view_id."'");
$company_row = mysqli_fetch_array($cumpany_sql);

// Ensure proper value assignment
$riyada_certificate = trim($company_row['riyada_status']);
$exp_date = $company_row['exp_date_formatted'];

// Company Details
$name = $company_row['name'];
$name_ar = $company_row['name_ar'];
$pobox = $company_row['pobox'];
$zipcode = $company_row['zipcode'];
$address = $company_row['address'];
$address_ar = $company_row['address_ar'];
$country = $company_row['country'];
$city = $company_row['city'];
$mobile_number = $company_row['mobile_number'];
$phone_number = $company_row['phone_number'];
$fax_number = $company_row['fax_number'];
$website_address = $company_row['website_address'];

// CR Details
$cr_number = $company_row['cr_number'];
$cr_legal_type = $company_row['cr_legal_type'];
$cr_registration_date = $company_row['cr_registration_date'];
$cr_expiry_date = $company_row['cr_expiry_date'];
$cr_tax_registration_number = $company_row['cr_tax_registration_number'];

// Login Details
$user_email = $company_row['user_email'];
$business_type_product_category = $company_row['business_type_product_category'];
$number_of_employee = $company_row['number_of_employee'];
$healthcare_status = $company_row['healthcare_status'];
$documents_req = $company_row['documents_req'];

// Documents
$upload_document1 = $company_row['upload_document1'];
$upload_document2 = $company_row['upload_document2'];
$upload_document3 = $company_row['upload_document3'];
...
<input type="text" class="form-control" id="gtin" name="name" value="<?php echo htmlspecialchars($name); ?>">
</div> 

<div class="form-group">
<label for="inputName" class="col-form-label">Company Name Arabic *</label>
<input type="text" class="form-control" name="name_ar" value="<?php echo htmlspecialchars($name_ar); ?>">
</div>


<div class="form-group">
<label for="inputName" class="col-form-label">PO Box *</label>
<input type="text" class="form-control" name="pobox" value="<?php echo htmlspecialchars($pobox); ?>">
</div>

<div class="form-group">
<label for="inputName" class="col-form-label">Zip/Postal Code *</label>
<input type="text" class="form-control" name="zipcode" value="<?php echo htmlspecialchars($zipcode); ?>">
</div>
...
<input type="text" class="form-control" name="address_ar" value="<?php echo htmlspecialchars($address_ar); ?>">
</div>
 </div>
...
<div class="form-group">
  ...
  <input type="text" class="form-control" name="mobile_number" value="<?php echo htmlspecialchars($mobile_number); ?>" required>
</div>
...
<div class="form-group">
  ...
  <input type="text" class="form-control" name="fax_number" value="<?php echo htmlspecialchars($fax_number); ?>">
</div>


<div class="form-group">
<label for="inputName" class="col-form-label">Website Address</label>
<input type="text" class="form-control" name="website_address" value="<?php echo htmlspecialchars($website_address); ?>">
</div>
...
<input type="text" class="form-control" placeholder="" name="cr_tax_registration_number" value="<?php echo htmlspecialchars($cr_tax_registration_number); ?>" required>
</div>
<?php
// $company_row['issue_date'] is in a different format, convert it
$issue_date_formatted = date("Y-m-d", strtotime($company_row['issue_date']));
?>
<!--<div class="form-group issue_date">
    <label>Issue Date*</label>
    <input type="date" class="form-control issue_date" placeholder="" name="issue_date" value="<?php //echo $issue_date_formatted; ?>">
</div>-->

<div  class="form-group">
<label for="inputName" class="col-form-label">Support documents required*</label>
<input type="file" class="form-control" id="payment_receipt" value="<?php echo htmlspecialchars($documents_req); ?>" name="documents_req">
<small><a href="<?php echo 'https://gs1oman.com/images/Upload/'.$documents_req; ?>"><?php  echo 'images/Upload/'.$documents_req; ?></a></small>
</div>	
...
<input type="text" class="form-control"   name="cr_number" value="<?php echo htmlspecialchars($cr_number); ?>" >
</div>
...
<input type="date" class="form-control" name="cr_registration_date" value="<?php echo htmlspecialchars($cr_registration_date); ?>" >
</div>

<div class="form-group">
<label for="inputName" class="col-form-label">CR Expiry Date</label>
<input type="date" class="form-control" name="cr_expiry_date" value="<?php echo htmlspecialchars($cr_expiry_date); ?>" >
</div>
...
<div class="form-group">
<label for="inputName" class="col-form-label">Chamber of Commercial Certificate</label>
<input type="file" class="form-control" name="upload_document2" value="<?php echo htmlspecialchars($upload_document2); ?>">
<small><a href="<?php echo $base_url.$upload_document2;?>"><?php  echo $upload_document2; ?></a></small>
</div>

<div class="form-group">
<label for="inputName" class="col-form-label">ARE YOU IN HEALTHCARE?</label>

<input type="radio" name="healthcare_status" value="Yes" <?php echo ($healthcare_status == 'Yes') ? 'checked' : ''; ?> class="tick" required> &nbsp; Yes
<input type="radio" name="healthcare_status" value="No" <?php echo (strcasecmp($healthcare_status, 'No') == 0) ? 'checked' : ''; ?> class="tick" required>&nbsp; No
...
<div class="form-group">
    ...
    <select class="form-control" name="riyada_certificate" id="riyada_certificate" required>
        <option value="">Select</option>
        <option value="Yes" <?php echo (strtolower(trim($riyada_certificate)) === 'yes') ? 'selected' : ''; ?>>Yes</option>
        <option value="No" <?php echo (strtolower(trim($riyada_certificate)) === 'no') ? 'selected' : ''; ?>>No</option>
    </select>
</div>

<div class="form-group expiry_date" style="display: <?php echo (strtolower(trim($riyada_certificate)) === 'yes') ? 'block' : 'none'; ?>">
    <label>Expiry Date*</label>
    <input type="date" class="form-control expiry_date" name="exp_date" value="<?php echo htmlspecialchars($exp_date); ?>" <?php echo (strtolower(trim($riyada_certificate)) === 'yes') ? 'required' : ''; ?>>
</div>

<div class="form-group">
<label for="inputName" class="col-form-label">User Name *</label>
<input type="text" class="form-control"   name="user_email" value="<?php echo htmlspecialchars($user_email); ?>" >
</div>

<div class="form-group">
<label for="inputName" class="col-form-label">Password *</label>
<input type="password" class="form-control"   name="password" value="<?php echo htmlspecialchars($cpassword); ?>" >
</div>
...
<option value="Agriculture" <?=($business_type_product_category=='Agriculture')?'selected':'';?>>Agriculture</option>
...
<option value="Disposable Polystrene Items" <?=($business_type_product_category=='Disposable Polystrene Items')?'selected':'';?>>Disposable Polystrene Items</option>
...
<option value="Food and Drink" <?=($business_type_product_category=='Food and Drink')?'selected':'';?>>Food and Drink</option>
<option value="Food Manufacturing" <?=($business_type_product_category=='Food Manufacturing')?'selected':'';?>>Food Manufacturing</option>
...
<option value="Surgical Equipment" <?=($business_type_product_category=='Surgical Equipment')?'selected':'';?>>Surgical Equipment</option>
...
<option value="Water" <?=($business_type_product_category=='Water')?'selected':'';?>>Water</option>
 ...
 <div class="form-group">
 <label for="inputName" class="col-form-label">Number of Employees</label>
 <input type="text" class="form-control"   name="number_of_employee" value="<?php echo htmlspecialchars($number_of_employee); ?>" >
 </div>

<div class="form-group">
<label for="inputName" class="col-form-label">Commercial Registrations</label>
<input type="file" class="form-control" name="upload_document1" value="<?php echo htmlspecialchars($upload_document1); ?>">
<small><a href="<?php echo $base_url.$upload_document1;?>"><?php  echo $upload_document1; ?></a></small>
</div>


<div class="form-group">
<label for="inputName" class="col-form-label">Other Documnets</label>
<input type="file" class="form-control" name="upload_document3" value="<?php echo htmlspecialchars($upload_document3); ?>">
<small><a href="<?php  echo $base_url.$upload_document3; ?>"><?php  echo $upload_document3; ?></a></small>
</div>
...
<input type="hidden" class="form-control"  name="id[]"  value="<?php echo htmlspecialchars($cumpany_contact_fire['id']); ?>" >
<div class="form-group">
<label for="inputName" class="col-form-label">First Name</label>
<input type="text" class="form-control"   name="first_name[]"  value="<?php echo htmlspecialchars($cumpany_contact_fire['first_name']); ?>" >
</div>

<div class="form-group">
<label for="inputName" class="col-form-label">Last Name</label>
<input type="text" class="form-control"   name="last_name[]" value="<?php echo htmlspecialchars($cumpany_contact_fire['last_name']); ?>" >
</div>
...
<input type="email" class="form-control"   name="email_id[]" value="<?php echo htmlspecialchars($cumpany_contact_fire['email_id']); ?>" >
</div>

<div class="form-group">
<label for="inputName" class="col-form-label">Phone Number</label>
<input type="text" class="form-control" name="phone_number1[]"   value="<?php echo htmlspecialchars($cumpany_contact_fire['phone_number1']); ?>" >
</div>
...
<label for="inputName" class="col-form-label">ARE YOU IN HEALTHCARE?</label>
<div class="col-md-12">
<input type="radio" name="healthcare_status" value="Yes" //($healthcare_status=='Yes')?'checked':''; ?> class="tick" required> &nbsp; Yes 
<input type="radio" name="healthcare_status" value="No" //($healthcare_status=='No')?'checked':'';?> class="tick" required> &nbsp; No
</div>
</div>-->
...
<a href="registration.php?page=REV" onclick="history.go(-1)";><input type="button" name="cancel" class="btn btn-warning" value="Back"></a>
</div>
</div>
...
$(document).ready(function() {
    ...
    function toggleExpiryDateField() {
        var riyadaValue = $('#riyada_certificate').val();
        if (riyadaValue && riyadaValue.toLowerCase() === 'yes') {
            $('.expiry_date').show();
            $('.expiry_date input').prop('required', true);
        } else {
            $('.expiry_date').hide();
            $('.expiry_date input').prop('required', false);
            if (riyadaValue.toLowerCase() === 'no') {
                $('.expiry_date input').val('');
            }
        }
    }
    ...
    $('#riyada_certificate').change(toggleExpiryDateField);
});
</script>
