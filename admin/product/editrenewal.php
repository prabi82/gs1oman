...
<!-- End of SECTION / BACKGROUND -->
	...
	{
	 $sql4="UPDATE `order_tbl` SET `prefix_num`='$prefix_num', `gln_number`='$gln_number',`gtins_annual_fee`='$gtins_annual_fee',`gln_price`='$gln_price', `sscc_price`='$sscc_price',`annual_subscription_fee`='$annual_subscription_fee',`annual_total_price`='$annual_subscription_fee',`certificate_img`='$c_img_path', `certificate_pdf`='$c_pdf_path', `status`='$status' WHERE company_id='".$view_id."' && id='".$id."' ";

	$query4=mysqli_query($conn,$sql4)or die(mysqli_error($conn));

	 if($query4){
		 $_SESSION['message']="Record Updated Successfully";
	echo "<script>window.location='https://gs1oman.com/admin/product/show.php?page=PROT';</script>";
	...
	echo "<script>window.location='common.php?stype=0&search=Filter&page=PROT';</script>";
...
echo "<script>window.location='common.php?stype=2&search=Filter&page=PROT';</script>";
...
echo "<script>window.location='common.php?stype=2&search=Filter&page=PROT';</script>";
	...
	echo "<script>window.location='https://gs1oman.com/admin/product/show.php?page=PROT';</script>";
        ...
        echo "<script>window.location='common.php?stype=3&search=Filter&page=PROT';</script>";
...
<div class="content-wrapper">
    ...
    <h3>Product Details Update On Renewal</h3>
...
?>
    ...
    <input type="text" class="form-control glnprice" name="gln_price" id="glnprice" value="<?=$product_row['gln_annual_fee'];?>" readonly >
    <input type="checkbox" onclick="amount()" name="gln_annual_fee" id="glncheck" class="test" value="<?=$productData['gln_price']?>">
    </div>
    </div>
    <div class="form-group">
    <label for="inputName" class="col-form-label">Annual Subscription Fee</label>
    <input type="text" class="form-control annualsubscriptionfee"  name="annual_subscription_fee"  id="annualsubscriptionfee" value="0" readonly >
    </div>
    </div>
    <div class="col-xl-6 col-lg-6 col-md-6 col-sm-6">
    <div class="form-group">
    ...
    <div class="form-group">
        ...
        <div class="input-group">
        <input type="text" class="form-control annualamount"  id="gtnprice" value="<?=$product_row['gtins_annual_fee'];?>" readonly>
        <input type="checkbox" onclick="amount()" id="gtincheck" name="gtins_annual_fee"  class="test" value="<?=$product_row['gtins_annual_fee'];?>" >
...
<div class="form-group">
    ...
    <div class="input-group">
    <input type="text" class="form-control" id="ssccamount"  name="sscc_price" value="<?=$product_row['sscc_annual_fee'];?>" readonly >
    <input type="checkbox"  onclick="amount()" name="sscc_annual_fee" id="sscccheck" class="test" value="<?=$productData['sscc_price']?>">
    </div>
</div>
 <div class="form-group">
    <label for="inputName" class="col-form-label">Annual Total Price</label>
    <input type="text" class="form-control"  name="annual_subscription_fee"  id="annualsubscriptionfeetotal" value="0" readonly >
    </div>
<!--<div class="form-group">
    <label for="inputName" class="col-form-label">Annual Total Price</label>
...
<div class="row gutters">
    <div class="col-xl-12">
    <input type="submit" id="submit" name="submit" class="btn btn-primary" value="Update On Renewal">
...
var gtncheckbox	=document.getElementById("gtincheck");
	if(gtncheckbox.checked==true){
		
		var gtn=document.getElementById("gtnprice").value;
	}else{
		var gtn=0;
	}
	var glncheckbox	=document.getElementById("glncheck");
	if(glncheckbox.checked==true){
		var gln=document.getElementById("glnprice").value;
	}else{
		var gln=0;
	}
	var ssccbox	=document.getElementById("sscccheck");
	if(ssccbox.checked==true){
		var sscc=document.getElementById("ssccamount").value;
	}else{
		var sscc=0;
	}
	
	var pasreGtn = parseInt(gtn);
	var pasreGln = parseInt(gln); 
	var pasreSSCC =  parseInt(sscc);
	
	var annualsubfee = pasreGtn + pasreGln + pasreSSCC;
	document.getElementById("annualsubscriptionfee").value=annualsubfee;
	document.getElementById("annualsubscriptionfeetotal").value=annualsubfee;
